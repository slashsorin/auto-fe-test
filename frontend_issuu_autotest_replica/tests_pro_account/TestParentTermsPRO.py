...
class TestParentTermsPRO(SeleniumTestCase.SeleniumTestCase):
    
    def test_parent_terms(self):
        try:
            sel = self.selenium
            sel.set_speed("500")
            sel.open("/about/parentterms")
            ...
            self.failUnless(sel.is_text_present("Parent Account Terms of Service"))
...
globals().update(make_platform_classes.make_platform_classes(TestParentTermsPRO))
