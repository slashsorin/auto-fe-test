...
class TestBookmarksReceivedPRO(SeleniumTestCase.SeleniumTestCase):
    ...
    def test_bookmarks_received(self):
        try:
            sel = self.selenium
            sel.set_speed("500")
            sel.open("/PROaccount/bookmarksreceived")
            ...
            sel.wait_for_page_to_load("60000")
            self.assertEqual("Issuu - proaccount - Bookmarks received", sel.get_title())
...
globals().update(make_platform_classes.make_platform_classes(TestBookmarksReceivedPRO))
