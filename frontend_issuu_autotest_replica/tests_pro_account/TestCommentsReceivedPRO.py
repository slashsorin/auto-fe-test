...
class TestCommentsReceivedPRO(SeleniumTestCase.SeleniumTestCase):

    def test_comments_received(self):
        try:
            sel = self.selenium
            sel.set_speed("500")
            sel.open("/PROaccount/commentsreceived")
...
globals().update(make_platform_classes.make_platform_classes(TestCommentsReceivedPRO))
