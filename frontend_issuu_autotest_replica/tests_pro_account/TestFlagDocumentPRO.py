...
class TestFlagDocumentPRO(SeleniumTestCase.SeleniumTestCase):

    def test_flag_document(self):
        try:
            sel = self.selenium
            sel.open("/login?onLogin=http%3A%2F%2F" + cfg.config['base-url'] + "%2Fflag%2Fdocument")
...
globals().update(make_platform_classes.make_platform_classes(TestFlagDocumentPRO))
