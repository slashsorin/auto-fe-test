...
class TestStyleBeauty(SeleniumTestCase):
   
    
    def test_style_beauty(self):
        try:
            sel = self.selenium
            sel.set_speed("3000")
            sel.open("/style-beauty")
...
globals().update(make_platform_classes.make_platform_classes(TestStyleBeauty))
